...
: D3DApp(hInstance), mLandVB(0), mLandIB(0), mWavesVB(0), mWavesIB(0), 
  ...
  mfxWorldViewProj(0), 
  mInputLayout(0), mEyePosW(0.0f, 0.0f, 0.0f), mTheta(1.5f*MathHelper::Pi), mPhi(0.1f*MathHelper::Pi), mRadius(160.0f)
{
	...
	// 点光 Point light--position is changed every frame to animate in UpdateScene function.
	...
	// 聚光灯 Spot light--position and direction changed every frame to animate in UpdateScene function.
...
void LightingApp::UpdateScene(float dt)
{
	...
	// Animate the lights.
	//
	// Circle light over the land surface.让点光源在地面之上转圈
...
void LightingApp::DrawScene()
{
	...
	// Set per frame constants. 将一个结构实例设置到一个effect变量实例上
...
XMFLOAT3 LightingApp::GetHillNormal(float x, float z)const
{
	// n = (-df/dx, 1, -df/dz)
	XMFLOAT3 n(
		-0.03f*z*cosf(0.1f*x) - 0.3f*cosf(0.1f*z), // GetHillHeight 函数的表达式对 x 求偏导所得的结果
		1.0f,
		-0.3f*sinf(0.1f*x) + 0.03f*x*sinf(0.1f*z));// GetHillHeight 函数的表达式对 z 求偏导所得的结果
