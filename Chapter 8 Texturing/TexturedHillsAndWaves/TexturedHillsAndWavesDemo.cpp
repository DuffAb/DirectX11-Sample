...
bool TexturedHillsAndWavesApp::Init()
{
	...
	HR(CreateDDSTextureFromFile(md3dDevice,
		L"Textures/grass.dds", 0, &mGrassMapSRV, 0, 0));

	/*HR(D3DX11CreateShaderResourceViewFromFile(md3dDevice, 
		L"Textures/water2.dds", 0, 0, &mWavesMapSRV, 0 ));*/
	HR(CreateDDSTextureFromFile(md3dDevice,
		L"Textures/water2.dds", 0, &mWavesMapSRV, 0, 0));
...
void TexturedHillsAndWavesApp::UpdateScene(float dt)
{
	...
	// Translate texture over time.根据时间平移纹理
	mWaterTexOffset.y += 0.05f*dt;
	mWaterTexOffset.x += 0.1f*dt;	
	XMMATRIX wavesOffset = XMMatrixTranslation(mWaterTexOffset.x, mWaterTexOffset.y, 0.0f);

	// Combine scale and translation.组合缩放和平移
