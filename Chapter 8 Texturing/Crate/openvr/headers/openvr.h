...
enum ETextureType
{
	TextureType_DirectX = 0, // Handle is an ID3D11Texture -- Handle 是一个 ID3D11Texture
	TextureType_OpenGL = 1,  // Handle is an OpenGL texture name or an OpenGL render buffer name, depending on submit flags -- Handle是OpenGL纹理名称或OpenGL渲染缓冲区名称，具体取决于提交标志
...
enum ETrackedDeviceClass
{
	...
	TrackedDeviceClass_HMD = 1,					// Head-Mounted Displays  头戴式显示器
	TrackedDeviceClass_Controller = 2,			// Tracked controllers    跟踪控制器
	TrackedDeviceClass_GenericTracker = 3,		// Generic trackers, similar to controllers  通用跟踪器，类似于控制器
	TrackedDeviceClass_TrackingReference = 4,	// Camera and base stations that serve as tracking reference points  用作跟踪参考点的相机和基站
};
...
{
	TrackingUniverseSeated = 0,		// Poses are provided relative to the seated zero pose ||| 姿势是相对于坐姿的零姿势
	TrackingUniverseStanding = 1,	// Poses are provided relative to the safe bounds configured by the user ||| 姿势是相对于用户设置的安全边界
...
// 此枚举中的每个条目都表示可以检索有关跟踪设备的属性。 许多字段只对一个 ETrackedDeviceClass 有效。
enum ETrackedDeviceProperty
{
	...
	// TrackedDeviceClass_TrackingReference 特有的属性
...
namespace vr
{
	...
	// IVRSystem 算是 OpenVR 環境下最基本的東西，要撰寫 OpenVR 的程式，一定要去初始化他；之後要取得控制器的狀態、以及其他相關的系統資訊，
	// 也都是要透過這個介面。實際要使用的時候，基本上就是先透過 vr::VR_Init() 這個函式來做初始化、並取得 vr::IVRSystem 的指標，
	// 之後再做後續的操作（話說，實際上的界面和官方文件的已經不同了…）；而最後結束時，再呼叫 vr::VR_Shutdown() 來把整個環境關閉。
class IVRSystem
{
public:
	...
	* From wiki：Provides the game with the minimum size that it should use for its offscreen render 
	...
	* distortion, and field of view.*/
	// 提供游戏的最小尺寸，它应该使用它画面外的渲染目标最小化像素的拉伸。
	// 这个尺寸与投影矩阵和变形功能相匹配，并根据分辨率、失真和视场的不同而变化。
	// pnWidth - 为渲染目标推荐的宽度
	// pnHeight - 为渲染目标推荐的高度
	virtual void GetRecommendedRenderTargetSize( uint32_t *pnWidth, uint32_t *pnHeight ) = 0;

	/** The projection matrix for the specified eye 
	* From wiki：Returns the projection matrix to use for the specified eye.*/
	// 返回指定的眼睛使用的投影矩阵。
	// eEye - Eye_Left或Eye_Right。确定哪个眼睛的功能应该返回投影。
	// fNearZ - 眼睛到近夹面的距离（单位：米）
	// fFarZ - 眼睛到远夹面的距离（单位：米）
	virtual HmdMatrix44_t GetProjectionMatrix( EVREye eEye, float fNearZ, float fFarZ ) = 0;

	/** The components necessary to build your own projection matrix in case your
	* application is doing something fancy like infinite Z 
	* From wiki：Returns the raw project values to use for the specified eye. 
	* Most games should use GetProjectionMatrix instead of this method, but sometimes 
	* a game needs to do something fancy with its projection and can use these values to compute its own matrix.
	*
	* The values represent the tangents of the half-angles from the center view axis. 
	* These values can be turned into a projection matrix with the function below.*/
	// 返回指定的眼睛的原始投影值。大多数游戏应该使用GetProjectionMatrix而不是这个方法，
	// 但是有时候游戏需要用它的投影来做一些奇妙的事情，并且可以使用这些值来计算它自己的矩阵。
	//
	// 这些值表示从中心视图轴到半角的切线。这些值可以被转换成具有下面函数的投影矩阵。
	...
	//}
	// 如果你使用OpenGL，你可以通过调用 glFrustum 来完成同样的事情。请注意，glFrustum(和类似的api)通常希望left、right、top和bottom的值
	// 是在近剪切平面上的坐标。为了满足这一要求，您必须将 GetProjectionRaw 返回的值乘以近距离(在上面的示例中zNear)。
	// eEye - Eye_Left或Eye_Right，确定哪个眼睛应该返回投影。
	...
	* Returns true for success. Otherwise, returns false, and distortion coordinates are not suitable. */
	// 在畸变映射表中获取单个畸变值的结果。输入值 fU、fV 是单个眼睛的视口，输出UVs是为源渲染目标在畸变着色器。
	// 成功的返回true，False表示值不合适。
	// eEye - Eye_Left或Eye_Right，确定哪个眼睛应该返回失真值
	// fU - 水平纹理坐标在视口内的输出像素
	// fV - 垂直纹理坐标在视口内的输出像素
	// pDistortionCoordinates - 用畸变值更新结构
	// ComputeDistortion 使用以下结构来返回畸变值:DistortionCoordinates_t { float rfRed[2]; float rfGreen[2]; float rfBlue[2]; };
	// rfRed包含了红色通道的UVs, rfGreen包含绿色通道的UVs, rfBlue包含蓝色通道的UVs。
	...
	*/
	// 返回视图空间和眼睛空间之间的转换。眼睛空间是每个眼睛的视觉空间中提供立体视差。模型是 Model * View * Eye * Projection，而不是 Model * View * Projection。
	// 通常情况下，视图和眼睛将会相乘，并在应用程序中作为视图处理。
	// 这个矩阵包含了用户的瞳孔间距(IPD)。
	// eEye - Eye_Left或Eye_Right。确定哪个眼睛应该返回眼睛矩阵
	...
	**/
	// 将座席跟踪器坐标系统的零姿态设置为HMD的当前位置和偏航。 ResetSeatedZero 后将所有 GetDeviceToAbsoluteTrackingPose 调用传递给TrackingUniverseSeated作为原点
	// 将相对于这个新的零姿态。 新的零点坐标系不会改变Y轴的事实在现实世界中，所以在调用之后从GetDeviceToAbsoluteTrackingPose返回的下一个姿势
	// ResetSeatedZeroPose可能不完全是一个单位矩阵。
	// 注意：此功能覆盖用户以前保存的坐姿零位姿势，只能作为用户操作的结果调用。用户还可以通过OpenVR仪表板设置他们的坐姿零姿势。
	...
	* in the list, or the size of the array needed if not large enough. */
	// 获取给定类别的跟踪设备（例如控制器）的有序设备索引数组。设备相对于指定的跟踪设备从右到左排序（默认值：hmd - 绝对跟踪空间中传入-1）。
	// 返回列表中的设备数量，如果数组大小不够大，则返回所需的数组大小。
	virtual uint32_t GetSortedTrackedDeviceIndicesOfClass( ETrackedDeviceClass eTrackedDeviceClass, VR_ARRAY_COUNT(unTrackedDeviceIndexArrayCount) vr::TrackedDeviceIndex_t *punTrackedDeviceIndexArray, uint32_t unTrackedDeviceIndexArrayCount, vr::TrackedDeviceIndex_t unRelativeToTrackedDeviceIndex = k_unTrackedDeviceIndex_Hmd ) = 0;

	/** Returns the level of activity on the device. -- 返回设备上的活动级别。*/
	...
	* actual tracked device. */
	...
	// 返回一个浮点属性。 如果设备索引无效或属性不是浮点类型，则该函数将返回 0。
	...
	* null. Strings will always fit in buffers of k_unMaxPropertyStringSize characters. */
	// 返回一个字符串属性. 如果设备索引无效或属性不是字符串类型，则此函数将返回0。否则，它将返回持有该字符串所需的字节数的长度，包括结尾null。
	// 字符串将总是填充 k_unMaxPropertyStringSize 个字符的缓冲区。
	...
	* is invalid. */
	// 使用当前controller的状态填充提供的结构体 pControllerState. 如果controller索引无效，则返回false。
	...
	* and axis combination for 5ms. */
	// 在 controller 上触发一个单一的触觉脉冲。在此调用之后，应用程序可能不会在这个 controller 和5毫秒的轴组合上触发另一个触觉脉冲。
...
struct Compositor_FrameTiming
{
	...
	uint32_t m_nNumFramePresents;	// number of times this frame was presented -- 这个帧被呈现的次数
	uint32_t m_nNumMisPresented;	// number of times this frame was presented on a vsync other than it was originally predicted to -- 这个帧在一个vsync上呈现的次数，而不是最初预测的
	uint32_t m_nNumDroppedFrames;	// number of additional times previous frame was scanned out -- 扫描出前一帧的附加次数
	uint32_t m_nReprojectionFlags;

	/** Absolute time reference for comparing frames.  This aligns with the vsync that running start is relative to. */
	// 用于比较帧的绝对时间基准。 这与运行启动相对的vsync是一致的。
...
// 輸出 OpenGL 或 Direct3D 所繪製的 3D 內容的介面，透過 texture 接受繪製好的左右兩眼的畫面，並顯示在頭戴顯示器內。
//
// 它的操作邏輯，基本上就是在主迴圈中：
//
// 1.呼叫 WaitGetPoses() 來取得 VR 相關裝置的位置資訊
// 2.根據現有的資料繪製左右兩眼的畫面
// 3.修正鏡片造成的影響（distortion）
// 4.Submit 左右兩眼的畫面給 IVRCompositor
//
// 針對這部分，官方有提供「hello_opengl」這個範例可以作為參考。他這邊是使用 SDL 來作為 OpenGL 的框架；
// 其中對於 OpenGL 初學者來說比較麻煩的，應該會是他計算 distortion 的部分一定得用到 shader 才行…
class IVRCompositor
{
public:
	...
	*/
	...
	/** Get the last set of poses returned by WaitGetPoses. -- 获取WaitGetPoses返回的最后一组姿势。*/
	...
	* It is okay to pass NULL for either pose if you only want one of the values. */
	// 用于访问由 WaitGetPoses 一次返回的最后一组姿势的接口。如果 unDeviceIndex 不小于 k_unMaxTrackedDeviceCount，则返回 VRCompositorError_IndexOutOfRange，
	// 否则返回 VRCompositorError_None。如果只需要其中一个值，则可以将NULL传递给任一姿势。
	...
	*	- SharedTexturesNotSupported (application needs to call CreateDXGIFactory1 or later before creating DX device)-- 应用程序在创建DX设备之前需要调用CreateDXGIFactory1或更高版本
	...
	* Returns 0 when fading out from an app and the app's process Id when fading into an app. */
	// 返回渲染最后一帧的进程的进程ID（如果合成器本身渲染了帧，则返回0）。从应用程序淡出时返回0，当淡入应用程序时返回应用程序的进程ID。
	virtual uint32_t GetLastFrameRenderer() = 0;

	/** Returns true if the current process has the scene focus -- 如果当前进程具有场景焦点，则返回true */
...
namespace vr
{
	...
	// 2D 內容的控制，可以想成浮在空中的一個平面；這個平面的位置，可以是絕對位置，或是相對於控制器／頭戴顯示器的位置。
	// 它的用途主要應該是用來呈現圖形介面、或是簡單的資訊。
	//
	// 在使用時，則是先要透過 CreateOverlay() 這個函式，來建立一個 overlay，並使用 VROverlayHandle_t 來做之後的操作、管理；
	// 而在建立好之後，必要的動作基本上還包括了設定他的大小、設定他在空間中的位置、設定內容、顯示狀態。之後，如果有更新，就在去更新圖片的內容即可。
	//
	// 另外， IVROverlay 也提供了簡單的事件管理，可以把控制器當作滑鼠來操作。
	//
	// 這部分的範例程式，可以參考官方的「helloworldoverlay」，它裡面是使用 QT5 作為圖形介面，並透過 OpenGL Texture 的形式、來更新 OpenVR overlay 的內容。
	class IVROverlay
	{
	public:
		...
		*	by default this is the pid of the process that made the overlay */
		// 设置允许渲染到这个覆盖图的pid（创建者pid始终允许渲染），默认情况下这是进行覆盖的过程的pid
		...
		*	Sort order defaults to 0. */
		// 设置叠加层的渲染排序顺序。 叠加层呈现这个顺序：
		//		由场景应用程序拥有的叠加层
		//		某些其他应用程序拥有的叠加层
		// 在一个类别中，叠加层的排序顺序由低到高。具有相同排序顺序的叠加在距离HMD的距离上被回显到前面。
		...
		// 以米为单位设置叠加四边形的宽度。 默认情况下，叠加层在1米的四边形上呈现
		...
		* drawing the device. Overlays with this transform type cannot receive mouse events. */
		// 设置变换以在渲染模型组件网格上绘制叠加而不是四边形。 这只会在系统绘制设备时绘制。 使用此转换类型的叠加层不能接收鼠标事件。
		...
		*	glBindTexture
		*/
		// 绘制覆盖的纹理。 此功能只能由覆盖层的创建者或渲染器进程调用（请参阅SetOverlayRenderingPid）。
...
namespace vr
{
	...
	*/
	// 如果有HMD，则返回true。 这个检查是尽可能轻量级的可以在VR_Init / VR_Shutdown之外调用。 
	// 当应用程序想知道初始化VR是否有可能，但尚未准备好采取这一步骤时，应该使用它。
	...
	* use that as a key to look up their own localized error message. This function may be called outside of VR_Init()/VR_Shutdown(). */
	// 返回 EVRInitError 的英文字符串。应用程序应该调用 VR_GetVRInitErrorAsSymbol，并将其用作查找自己的本地化错误消息的关键字。 这个函数可以在VR_Init（）/ VR_Shutdown（）之外被调用。
	...
	*/
	...
	// 它保存了所有的VR接口类的对象，所有的这些接口类对象一起就称为Context。
	// 里面只有一个简单的逻辑就是每次使用某个接口前先通过VRToken判断接口是否有效，
	// 如果接口无效（接口为空或者token变了都表示接口无效），则调用 VR_GetGenericInterface 重新获取。
	...
	/** unloads vrclient.dll. Any interface pointers from the interface are
	* invalid after this point */
	// 卸载vrclient.dll。 在这之后，来自接口的任何接口指针都是无效的
	inline void VR_Shutdown()
	{
		VR_ShutdownInternal();
	}
