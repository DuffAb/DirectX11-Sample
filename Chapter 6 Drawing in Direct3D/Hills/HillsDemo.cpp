...
void HillsApp::BuildGeometryBuffers()
{
	...
	for(size_t i = 0; i < grid.Vertices.size(); ++i)
	{
		...
		p.y = GetHeight(p.x, p.z);//高度函数，修改y的值，形成山丘的形状

		vertices[i].Pos   = p;
		
		// Color the vertex based on its height.根据顶点高度设置颜色
		if( p.y < -10.0f )
		{
			// Sandy beach color.沙滩色
			vertices[i].Color = XMFLOAT4(1.0f, 0.96f, 0.62f, 1.0f);
		}
		else if( p.y < 5.0f )
		{
			// Light yellow-green.淡绿色
			vertices[i].Color = XMFLOAT4(0.48f, 0.77f, 0.46f, 1.0f);
		}
		else if( p.y < 12.0f )
		{
			// Dark yellow-green.深绿色
