...
bool ShapesApp::Init()
{
	...
	// 下面示范了如何通过创建一个光栅化状态块来禁用背面消隐   背面消隐（backface culling）是指让管线放弃对朝后的三角形的处理
	D3D11_RASTERIZER_DESC wireframeDesc;
	ZeroMemory(&wireframeDesc, sizeof(D3D11_RASTERIZER_DESC));
	wireframeDesc.FillMode = D3D11_FILL_WIREFRAME;// 表示以线框模式渲染几何体  Default:D3D11_FILL_SOLID
	wireframeDesc.CullMode = D3D11_CULL_BACK;	  // 表示消隐朝后的三角形      Default:D3D11_CULL_BACK
	wireframeDesc.FrontCounterClockwise = false;  // 表示按顺时针方向环绕的三角形（相对于观察者）是朝前的
	wireframeDesc.DepthClipEnable = true;

	// 创建ID3D11RasterizerState对象，第1个参数是一个指向D3D11_RASTERIZER_DESC结构体的指针，
	// 该结构体用于描述所要创建的光栅化状态块；第二个参数用于返回创建后的ID3D11RasterizerState对象
...
void ShapesApp::BuildFX()
{
	...
	// 创建一个effect
	HR(D3DX11CreateEffectFromMemory(&compiledShader[0], // pData：指向编译好的effect数据的指针
		size,       // DataLength：effect数据的长度，以字节为单位
		0,          // FXFlags：Effect标识必须与定义在D3DCompileFromFile方法中的Flags2匹配
		md3dDevice, // pDevice：指向Direct3D 11设备的指针
		&mFX));     // ppEffect：指向创建好的effect的指针
